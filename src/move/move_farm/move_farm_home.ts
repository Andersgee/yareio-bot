...
export default function move_farm_home(
  targetps: Vec2s,
  moving: Vec,
  farming: Vec,
  ...
  const selfers_max = Math.floor(
    maxfarmfraction * maxStarSelfers(stars.me, shipsize)
  );
...
function position3ships(targetps: Vec2s, moving: Vec, farming: Vec): number {
  ...
  const s0 = ship_closest(myships, points.h1, moving);
  targetps[s0.index] = staynearstructure(s0.position, points.h1, stars.me);
  moving.push(s0.index);
  farming.push(s0.index);

  const s1 = ship_closest(myships, points.h1, moving);
  targetps[s1.index] = staynearstructure(s1.position, points.h1, stars.me);
  moving.push(s1.index);
  farming.push(s1.index);

  const s2 = ship_closest(myships, points.h2, moving);
  targetps[s2.index] = offsetmax20(s2.position, points.h2);
...
function position2ships(targetps: Vec2s, moving: Vec, farming: Vec): number {
  ...
  const s2 = ship_closest(myships, points.h1, moving);
  targetps[s2.index] = offsetmax20(s2.position, points.h1);
  moving.push(s2.index);
  farming.push(s2.index);
